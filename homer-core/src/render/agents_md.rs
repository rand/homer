...
impl Renderer for AgentsMdRenderer {
    ...
    async fn render(
        ...
        let _ = writeln!(out, "# AGENTS.md");
        let _ = writeln!(out);
        let _ = writeln!(out, "{}", super::traits::staleness_header(store).await);
        let _ = writeln!(
            out,
...
async fn render_build_commands(
    ...
    let _ = writeln!(out, "## Build & Test Commands");
    let _ = writeln!(out);
    ...
    if !found_commands {
        ...
        if has_cargo {
            ...
            let _ = writeln!(out, "cargo build          # Build");
            let _ = writeln!(out, "cargo test           # Test");
            let _ = writeln!(out, "cargo clippy         # Lint");
            let _ = writeln!(out, "```");
            let _ = writeln!(out);
        }
        if has_npm {
            let _ = writeln!(out, "```bash");
            let _ = writeln!(out, "npm install          # Install");
            let _ = writeln!(out, "npm test             # Test");
            let _ = writeln!(out, "```");
            let _ = writeln!(out);
        }
        if has_python {
            let _ = writeln!(out, "```bash");
            let _ = writeln!(out, "pip install -e .     # Install");
            let _ = writeln!(out, "pytest               # Test");
            let _ = writeln!(out, "```");
            let _ = writeln!(out);
        }
        if has_go {
            let _ = writeln!(out, "```bash");
            let _ = writeln!(out, "go build ./...       # Build");
            let _ = writeln!(out, "go test ./...        # Test");
            let _ = writeln!(out, "```");
            let _ = writeln!(out);
        }

        if !has_cargo && !has_npm && !has_python && !has_go {
            let _ = writeln!(out, "*No build commands detected.*");
            let _ = writeln!(out);
        }
    }
...
/// Semantic patterns: directory substring → human-readable label.
...
fn derive_cluster_label(members: &[(String, f64)]) -> String {
    ...
    // Try semantic pattern matching against the dominant directory
    let dir_lower = dominant_dir.to_lowercase();
    ...
    // Fallback: title-case the last directory component
    dominant_dir
        .rsplit('/')
        .next()
        .map_or_else(|| "Mixed".to_string(), title_case)
}

/// Title-case a directory component: `my_module` → `My Module`.
...
async fn render_key_documents(
    ...
    let _ = writeln!(out, "## Key Documents");
    let _ = writeln!(out);

    let _ = writeln!(out, "| Document | Type | References |");
    let _ = writeln!(out, "|----------|------|----------:|");

    for (name, doc_type, refs) in &top_docs {
        let _ = writeln!(out, "| `{name}` | {doc_type} | {refs} |");
    }
    let _ = writeln!(out);
...
async fn render_module_map(out: &mut String, store: &dyn HomerStore) -> crate::error::Result<()> {
    ...
    if modules.is_empty() {
        let _ = writeln!(out, "*No modules detected.*");
        let _ = writeln!(out);
        return Ok(());
    }
    ...
    let _ = writeln!(out, "| Module | Files | Functions | Types |");
    let _ = writeln!(out, "|--------|------:|----------:|------:|");
    ...
    for module in &sorted_modules {
        ...
        let _ = writeln!(out, "| `{}` | {} | {} | {} |", module.name, files, fns, tps);
    }
    let _ = writeln!(out);
...
async fn render_change_patterns(
    ...
    let _ = writeln!(out, "## Change Patterns");
    let _ = writeln!(out);
    ...
    let _ = writeln!(out, "**Most frequently changed files:**");
    let _ = writeln!(out);
...
async fn render_load_bearing_code(
    ...
    let _ = writeln!(out, "## Load-Bearing Code");
    let _ = writeln!(out);
    let _ = writeln!(
        out,
    ...
    if salience_results.is_empty() {
        let _ = writeln!(out, "*No salience data available.*");
        let _ = writeln!(out);
        return Ok(());
    }
    ...
    let _ = writeln!(out, "| Entity | Salience | Classification |");
    let _ = writeln!(out, "|--------|----------|----------------|");
    ...
    for (node_id, val, cls) in top_n {
        ...
        let _ = writeln!(out, "| `{short_name}` | {val:.2} | {cls} |");
    }
    let _ = writeln!(out);
...
async fn render_danger_zones(out: &mut String, store: &dyn HomerStore) -> crate::error::Result<()> {
    let _ = writeln!(out, "## Danger Zones");
    let _ = writeln!(out);
    let _ = writeln!(
        out,
    ...
    if danger_files.is_empty() {
        let _ = writeln!(out, "*No danger zones detected.*");
        let _ = writeln!(out);
        return Ok(());
    }
    ...
    let _ = writeln!(out, "| File | Changes | Bus Factor | Top Contributor |");
    let _ = writeln!(out, "|------|--------:|-----------:|----------------:|");
...
async fn render_language_distribution(
    ...
    if !lang_counts.is_empty() {
        ...
        for (lang, count) in &sorted {
            let _ = writeln!(out, "- {lang}: {count} files");
        }
        let _ = writeln!(out);
    }
...
async fn render_dependency_count(
    ...
    if !deps.is_empty() {
        let _ = writeln!(out, "**Dependencies:** {} external packages", deps.len());
        let _ = writeln!(out);
    }
    Ok(())
}
        ...
        if conventions.len() > 1 {
            for conv in conventions.iter().take(4) {
                ...
                let count = conv
                    ...
                    .unwrap_or(0);
                if count > 0 {
                    let _ = writeln!(out, "- {name}: {count} ({pct:.0}%)");
                }
            }
        }
    }
...
async fn render_testing_conventions(
    ...
    let _ = writeln!(out, "**Testing:** {framework}");
    let _ = writeln!(
        out,
        "- Pattern: {pattern}{}",
...
async fn render_error_conventions(
    ...
    let _ = writeln!(out, "**Error handling:** {approach}");
    ...
    if let Some(patterns) = result.data.get("patterns").and_then(|v| v.as_array()) {
        for pat in patterns.iter().take(5) {
            ...
            let count = pat
                ...
                .unwrap_or(0);
            if count > 0 {
                let _ = writeln!(out, "- `{name}`: {count} occurrences");
            }
        }
    }
    let _ = writeln!(out);
...
async fn render_doc_conventions(
    ...
    if dominant != "none" {
        let _ = writeln!(out, "- Dominant style: {dominant}");
    }
    let _ = writeln!(out);
...
async fn render_agent_rules(
    ...
    if let Some(files) = rule_files {
        if !files.is_empty() {
            ...
            let _ = writeln!(out, "**Agent rules:** {}", names.join(", "));
        }
    }
    ...
    if let Some(drifted) = result.data.get("drifted").and_then(|v| v.as_array()) {
        for drift in drifted {
            ...
            let actual = drift
                ...
                .and_then(serde_json::Value::as_str)
                .unwrap_or("?");
            let _ = writeln!(out, "- **Drift:** states `{stated}` but {actual}");
        }
    }
...
async fn render_common_tasks(out: &mut String, store: &dyn HomerStore) -> crate::error::Result<()> {
    ...
    let _ = writeln!(out, "## Common Tasks");
    let _ = writeln!(out);
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(out, "| Task | Files Typically Involved | Frequency |");
    let _ = writeln!(out, "|------|------------------------|----------:|");
    ...
    for pattern in patterns.iter().take(10) {
        ...
        let _ = writeln!(out, "| {name} | `{files}` | {frequency} sessions |");
    }
    let _ = writeln!(out);
...
async fn render_confusion_zones(
    ...
    let _ = writeln!(out, "## Areas That Confuse Agents");
    let _ = writeln!(out);
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(out, "| File | Correction Rate | Corrections |");
    let _ = writeln!(out, "|------|----------------:|------------:|");

    for (name, rate, count) in zones.iter().take(10) {
        let _ = writeln!(out, "| `{name}` | {:.0}% | {count} |", rate * 100.0);
    }
    let _ = writeln!(out);
...
async fn render_domain_vocabulary(
    ...
    let _ = writeln!(out, "## Domain Vocabulary");
    let _ = writeln!(out);

    let _ = writeln!(out, "| Term | File | Key Entities | References |");
    let _ = writeln!(out, "|------|------|-------------|----------:|");
    ...
    for entry in vocabulary.iter().take(20) {
        ...
        let _ = writeln!(out, "| {term} | `{file}` | {entities} | {ref_count} |");
    }
    let _ = writeln!(out);
...
async fn render_key_design_decisions(
    ...
    let _ = writeln!(out, "## Key Design Decisions");
    let _ = writeln!(out);
    ...
    for adr in adrs.iter().take(limit) {
        let status = adr
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("accepted");
        let _ = writeln!(out, "- **{}** ({})", adr.name, status);
    }
    ...
    for result in rationale_results.iter().take(limit) {
        ...
        if !rationale.is_empty() {
            ...
            let _ = writeln!(out, "- **{short_name}**: {rationale}");
        }
    }
    let _ = writeln!(out);
...
mod tests {
    ...
    async fn render_architecture_and_documents() {
        ...
        assert!(
            ...
            "Should have cluster info: {content}"
        );
        assert!(
    ...
    #[test]
    fn semantic_label_for_analyze_paths() {
        let members = vec![
            ("homer-core/src/analyze/behavioral.rs".to_string(), 0.5),
            ("homer-core/src/analyze/centrality.rs".to_string(), 0.3),
            ("homer-core/src/analyze/community.rs".to_string(), 0.2),
        ];
        assert_eq!(derive_cluster_label(&members), "Analysis Engine");
    }

    #[test]
    fn semantic_label_for_extract_paths() {
        let members = vec![
            ("src/extract/git.rs".to_string(), 0.5),
            ("src/extract/structure.rs".to_string(), 0.3),
        ];
        assert_eq!(derive_cluster_label(&members), "Data Extraction");
    }

    #[test]
    fn semantic_label_fallback_to_title_case() {
        let members = vec![
            ("src/widgets/button.rs".to_string(), 0.5),
            ("src/widgets/slider.rs".to_string(), 0.3),
        ];
        assert_eq!(derive_cluster_label(&members), "Widgets");
    }
