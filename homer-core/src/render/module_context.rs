...
async fn render_all_module_contexts(
    ...
    let staleness = super::traits::staleness_header(db).await;
    let mut count = 0u32;
    ...
    for module in &modules {
        ...
        let content = render_single_module(dir, &data, &staleness);
...
fn render_single_module(dir: &str, data: &ModuleData, staleness: &str) -> String {
    let mut out = String::with_capacity(2048);

    let display_dir = if dir == "." { "(root)" } else { dir };
    let _ = writeln!(out, "# {display_dir}");
    let _ = writeln!(out);
    let _ = writeln!(out, "{staleness}");
    let _ = writeln!(out, "> Auto-generated by Homer for `{dir}/`");
    let _ = writeln!(out);
...
fn render_key_entities(out: &mut String, dir: &str, data: &ModuleData) {
    ...
    for (name, salience) in filtered {
        ...
        let _ = writeln!(out, "| `{short_name}` | {score_str} | {cls_str} |");
    }
    let _ = writeln!(out);
}
...
fn render_change_profile(out: &mut String, dir: &str, data: &ModuleData) {
    let _ = writeln!(out, "## Change Profile");
    let _ = writeln!(out);

    let Some(files) = data.module_files.get(dir) else {
        let _ = writeln!(out, "*No change data available.*");
        let _ = writeln!(out);
        return;
    };
    ...
    if total_changes > 0 {
        ...
        let _ = writeln!(out, "- **Total changes**: {total_changes} (avg {avg}/file)");
    }
    if min_bf < u64::MAX {
        let _ = writeln!(out, "- **Min bus factor**: {min_bf}");
    }
    ...
    if max_sal > 0.0 {
        let _ = writeln!(out, "- **Peak salience**: {max_sal:.2}");
    }
    ...
    if let Some(partners) = data.co_changes.get(dir) {
        if !partners.is_empty() {
            ...
            let _ = writeln!(out, "- **Co-changes with**: {}", display.join(", "));
        }
    }
...
fn render_conventions(out: &mut String, dir: &str, data: &ModuleData) {
    ...
    let _ = writeln!(out);
    let _ = writeln!(out, "- **Naming**: {convention} (project-wide)");
    let _ = writeln!(out);
}
...
fn render_recent_changes(out: &mut String, dir: &str, data: &ModuleData) {
    ...
    let _ = writeln!(out, "## Recent Significant Changes");
    let _ = writeln!(out);

    for (label, file) in commits.iter().take(5) {
        let _ = writeln!(out, "- {label} (`{file}`)");
    }
    let _ = writeln!(out);
}
