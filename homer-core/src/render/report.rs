...
fn render_html(data: &ReportData) -> String {
    ...
    writeln!(h, "<footer><p>Generated by Homer</p></footer>").unwrap();
    writeln!(h, "</body>").unwrap();
    writeln!(h, "</html>").unwrap();

    h
}

fn render_executive_summary(h: &mut String, data: &ReportData) {
    writeln!(h, "<h1>Homer Report</h1>").unwrap();
    writeln!(h, "<section class=\"summary\">").unwrap();
    writeln!(h, "<h2>Executive Summary</h2>").unwrap();
    writeln!(h, "<div class=\"metrics\">").unwrap();
...
fn render_architecture_diagram(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Architecture Diagram</h2>").unwrap();
    writeln!(h, "<div class=\"mermaid\">").unwrap();
    writeln!(h, "graph TD").unwrap();
    ...
    for (cid, members) in &sorted {
        ...
        writeln!(h, "  subgraph C{cid}[\"{dir_label}\"]").unwrap();
        for (idx, member) in members.iter().take(5).enumerate() {
            let short = member.rsplit('/').next().unwrap_or(member);
            writeln!(h, "    N{cid}_{idx}[\"{short}\"]").unwrap();
        }
        if members.len() > 5 {
            writeln!(h, "    N{cid}_more[\"...+{}\"]", members.len() - 5).unwrap();
        }
        writeln!(h, "  end").unwrap();
    }
...
fn render_hotspot_map(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Hotspot Map</h2>").unwrap();
    writeln!(h, "<p>Top entities by composite salience score.</p>").unwrap();
    writeln!(
        h,
...
fn render_coupling_section(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Coupling Analysis</h2>").unwrap();
    writeln!(h, "<p>Files that frequently change together.</p>").unwrap();
    writeln!(
        h,
...
fn render_trends_section(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Centrality Trends</h2>").unwrap();
    writeln!(h, "<p>Entities with changing structural importance.</p>").unwrap();
    writeln!(
        h,
...
fn render_risk_section(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Risk Assessment</h2>").unwrap();
    writeln!(
        h,
...
fn render_doc_health(h: &mut String, data: &ReportData) {
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Documentation Health</h2>").unwrap();
    writeln!(h, "<div class=\"metrics\">").unwrap();
    emit_metric(
        h,
        "Coverage",
...
fn render_agent_effectiveness(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Agent Effectiveness</h2>").unwrap();

    if !data.correction_hotspots.is_empty() {
        writeln!(h, "<h3>Correction Hotspots</h3>").unwrap();
        writeln!(h, "<p>Files where agent corrections are most frequent:</p>").unwrap();
        writeln!(h, "<table><tr><th>Entity</th><th>Score</th></tr>").unwrap();
        ...
        writeln!(h, "</table>").unwrap();
    }

    if !data.prompt_hotspots.is_empty() {
        writeln!(h, "<h3>Prompt Hotspots</h3>").unwrap();
        writeln!(
            h,
...
fn render_team_topology(h: &mut String, data: &ReportData) {
    ...
    writeln!(h, "<section>").unwrap();
    writeln!(h, "<h2>Team Topology</h2>").unwrap();
    writeln!(
        h,
